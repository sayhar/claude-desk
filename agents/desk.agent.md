...
3. Read `agents/state/desk/operational-notes.md` if it exists -> Know persistent patterns and intel -> Report: "Operational notes loaded" OR "No operational notes yet"
4. Skim `me/working-understanding.md` (CORE section only -- stop at `# REFERENCE` header if present, otherwise skim briefly) -> Report: "Context loaded"
5. Read `ops/dashboard.md` -> Report current status (active items, blockers, what's next)
6. Read `data/actions.md` if it exists -> Report top priorities
7. Read FIRST 50 LINES of last 2 session notes from `agents/state/sessions/desk/` -> Report: "Recent sessions: [dates]. [one-line summary of each]"
